...
struct mystring
{
	...
	size_t Size;//the max size of the string
};
...
int str_ncopy(myString s1,myString s2,size_t n)
{
	...
	int i;
	for(i = 1; i < n; s1->str[i] = s2->str[i], i++);
	s1->len = i;
	return 1;
}

void str_end_cat(myString s1,const myString s2)
{
	int i,j;
	size_t tem;
	tem = s1->len + s2->len; 
...
double str_sTodouble(const myString s)
{
	if(s->len == 0)
		return 0;
	int i,j;
	double ret1 = 0;//integer part
	double ret2 = 0;//decimal part
	int flag = 1;
	i = 0;
	if(s->str[i] == '-')
	{
...
{
	char ** ans;//define a two-dimensional char array pointer
	int i,index;
	size_t nSize,count;
	count = 0;
	nSize = INIT_SIZE;

	ans = (char **)malloc(sizeof(char*)*nSize);//assign the appropriate size
	if(ans == NULL)
	{
	...
	while((index = find_first_str(s,ch)) != -1)
	{
		...
		if(count >= nSize)//if exceed initial max size, then expend original dize 
		{
			char ** temp;
